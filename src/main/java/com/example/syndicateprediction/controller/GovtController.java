...
public class GovtController {
    ...
    public ResponseEntity<DefaultMessage> editProduct(@RequestBody ProductRequest productRequest,@PathVariable Integer product_id) throws DefaultException {
        val day= LocalDateTime.now().getDayOfMonth();
        val month=LocalDateTime.now().getMonthValue();
        val year=LocalDateTime.now().getYear();
        Product product = Product.builder()
                ...
                .year(year)
                .build();
        return ResponseEntity.status(201).body(productService.editProduct(product,product_id));
    }
